...
# The domain-keyed script, not a per-site `pa-<id>.js`. Those ids are minted
# when a site is created in Plausible and are NOT restored by a backup or
# reissued when a site is recreated, so the old id 404s forever and records
# nothing while the page looks perfectly fine. data-domain has no such
# failure mode, and it drops the inline init block, which lets the CSP go
# back to having no script hash at all.
ANALYTICS_SRC = f"{ANALYTICS_HOST}/js/script.js"
ANALYTICS_DOMAIN = "plaintext.report"
...
def content_security_policy():
    return "; ".join([
        "default-src 'none'",
        "style-src 'self'",
        f"script-src 'self' {ANALYTICS_HOST}",
...
def render_html(sections, feeds, hours, now, failed_names, analytics=True,
    ...
    if analytics:
        ...
        add(f'<script defer data-domain="{html.escape(ANALYTICS_DOMAIN, quote=True)}" '
            f'src="{html.escape(ANALYTICS_SRC, quote=True)}"></script>')
...
# The analytics tag is domain-keyed, so there is no inline script and the
# policy needs no hash: nothing inline is allowed to run at all.
